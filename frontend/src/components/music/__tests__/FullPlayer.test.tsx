...
describe('FullPlayer', () => {
  ...
  describe('曲库视图模式', () => {
    ...
    it('收藏音乐按钮应该显示收藏数量', () => {
      ...
      expect(badge?.textContent).toBe('1'); // mockUseFavorites.favoritesCount
    });
  });
  ...
  describe('封面图片加载失败处理', () => {
    it('应该显示默认封面图片', () => {
      ...
      // 验证 src 被更新为默认图片
      expect(coverImage).toHaveAttribute('src', 'https://picsum.photos/256/256');
    });
  });
  ...
  describe('交互测试', () => {
    ...
    it('多个视图模式切换应该正确工作', async () => {
      render(<FullPlayer onClose={mockOnClose} />);

      // 全部歌曲 -> 歌手视图
      fireEvent.click(screen.getByText('歌手视图'));
      await waitFor(() => {
        expect(screen.getByTestId('artist-view')).toBeInTheDocument();
      });

      // 歌手视图 -> 收藏音乐
      fireEvent.click(screen.getByText('收藏音乐'));
      await waitFor(() => {
        expect(screen.getByTestId('favorite-view')).toBeInTheDocument();
      });

      // 收藏音乐 -> 全部歌曲
      fireEvent.click(screen.getByText('全部歌曲'));
      await waitFor(() => {
        expect(screen.getByTestId('all-songs-view')).toBeInTheDocument();
      });
    });
