...
import { on, clearEventListeners } from '../services/socketService';
export const useGameState = () => {
    const [gameState, setGameState] = useState(null);
    const [messages, setMessages] = useState([]);
    const [rooms, setRooms] = useState([]);
    const [playerColor, setPlayerColor] = useState(null);
    const [isSpectator, setIsSpectator] = useState(false); // 观战标记
    const [playerLeftNotice, setPlayerLeftNotice] = useState(null);
    const joinedRoom = useCallback((color, initialGameState) => {
    ...
    const watchingRoom = useCallback((initialGameState) => {
        setPlayerColor(null);
    ...
    const updateRooms = useCallback((roomList) => {
        setRooms(roomList);
    }, []);
    ...
    useEffect(() => {
        ...
        const handleRoomListUpdate = (roomList) => {
